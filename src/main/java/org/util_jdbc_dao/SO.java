...
public class SO {
	...
	public static void writeSmallBinaryFile(byte[] aBytes, String aFileName)
			throws IOException {
	...
	public static String createFolder(String path) throws Exception {
		...
		if (file.exists() == false) {
			...
			if (b) {
				...
				r = "[ERROR]\t";
				System.out.println(r + " CREATE FOLDER "
	 ...
	 */
	private static void copy(File FOrigen, File FDestino) {
	...
	private static void copyFile(File FOrigen, File FDestino) {
		try {
			...
			if (FOrigen.exists()) {
				...
				System.out.println("[ERROR]\tCOPY FILE - FILE DOES NOT EXIST. "
